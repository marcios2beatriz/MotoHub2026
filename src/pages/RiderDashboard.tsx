...
export default function RiderDashboard() {
  ...
  const handleLaunchDelivery = async (e: React.FormEvent) => {
    ...
    if (isNaN(finalVal) || finalVal <= 0) {
      alert('Erro: O valor daRobô de entrega deve ser maior que zero.');
      return;
    }
                ...
                {filteredEarningsDeliveries.length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-100 bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    {filteredEarningsDeliveries.map(del => {
                      ...
                      return (
                        <div key={del.id} className={`p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 hover:bg-slate-50 transition-colors ${repeatCount > 1 ? 'border-l-4 border-l-amber-500 bg-amber-50/20' :<dyad-write path="src/pages/RiderDashboard.tsx" description="Finalizar código completo do painel do motoboy garantindo a contagem e visualização de todas as 12 corridas">
"use client";
...
export default function RiderDashboard() {
  ...
  const filteredEarningsDeliveries = deliveries.filter(d => {
    ...
    if (earningsFeatureFilter === 'same_order_number') {
      ...
      const isStandard = d.deliveryType === 'same_address' || Boolean(d.linkedOrderNumber) || (d.additionalValue && Number(d.additionalValue) > 0);
      if (isStandard) return false;
    }
