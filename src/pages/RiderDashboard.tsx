                      ...
                      {historyDeliveries.map((del) => {
                        ...
                        return (
                          <div key={del.id} className={`py-3.5 space-y-2 ${isSame ? 'bg-purple-50/30 p-3 rounded-xl' : ''}`}>
                            ...
                            {/* SINALIZAÇÃO VISUAL DE COBRANÇA NO HISTÓRICO */}
...
<dyad-write path="src/utils/db.ts" description="Ajustando tipagem estrita de paymentMethod no pullFromSupabase">
"use client";
...
function scanAllLocalStorageDeliveries(): Map<string, { orderNumber?: string; notes?: string; customerChat?: string; additionalValue?: number; additionalReason?: string; linkedOrderNumber?: string; deliveryType?: 'standard' | 'same_address'; paid?: boolean; paymentMethod?: Delivery['paymentMethod']; orderCollectionAmount?: number; changeFor?: number }> {
            ...
            if (item && item.id) {
              ...
              if (num || item.notes || item.paid !== undefined || item.paymentMethod) {
                ...
                recoveredMap.set(item.id, {
                  ...
                  paymentMethod: (item.paymentMethod as Delivery['paymentMethod']) || existing.paymentMethod,
    ...
    try {
      ...
      if (delData) {
        ...
        memoryDeliveries = delData.map(d => {
          ...
          let paymentMethod: Delivery['paymentMethod'] = fromStorage?.paymentMethod || fromMemory?.paymentMethod || 'already_paid';
          ...
          if (d.order_number && d.order_number.startsWith('{')) {
            try {
              ...
              if (parsed.pm) {
                paymentMethod = parsed.pm as Delivery['paymentMethod'];
              }
